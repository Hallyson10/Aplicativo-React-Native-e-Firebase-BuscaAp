...
import {
  ...
  LocationCep,
  LocationMap,
} from '../../Store/Actions/Vagas/Vagas/PropertiesVagas';
...
class AboutLocation extends PureComponent {
  ...
  find = city => {
    if (city === 'rua') {
      ...
      if (this.props.cidade != '') {
        Geocoder.init(GOOGLE_MAPS_APIKEY);
        Geocoder.from(this.props.cidade)
          .then(json => {
            var location = json.results[0].geometry.location;
            var long = location.lng;
            var lat = location.lat;
      ...
      <KeyboardAvoidingView
        ...
        <View
          ...
          <Text style={styles.informacoes}>
            Informações sobre a localização facilita a busca por uma moradia!
          </Text>
        </View>

        <TextInput
          style={this.state.erroCity ? styles.inputErro : styles.input}
        ...
        />
        <TextInput
          style={this.state.erroRua ? styles.inputErro : styles.input}
          placeholder="Ex:Rua Jose Bastos 234"
          value={this.props.rua}
          onChangeText={rua => {
            this.props.LocationRua(rua);
          }}
        ...
        <TextInput
          ...
          onChangeText={bairro => {
            this.props.LocationBairro(bairro);
          }}
        />
        <View style={{marginTop: -8}}>
          {this.state.erroCity || this.state.erroRua ? (
            <Text style={{color: 'red', fontSize: 12}}>
              {this.state.op} Não encontrada{' '}
            </Text>
          ) : null}
        </View>
      </KeyboardAvoidingView>
...
const mapStateToProps = state => ({
  cidade: state.Properties.cidade,
  bairro: state.Properties.bairro,
  rua: state.Properties.rua,
  latitude: state.Properties.latitude,
  longitude: state.Properties.longitude,
});
