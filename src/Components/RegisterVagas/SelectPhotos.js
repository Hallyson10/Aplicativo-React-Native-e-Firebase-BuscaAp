...
class SelectPhotos extends PureComponent {
  ...
  Save = async () => {
    await this.props.navigation.navigate('RegisterVacance');
    ...
    try {
      ImagePicker.launchCamera(options, response => {
        if (response.didCancel) {
          ...
          this.props.navigation.navigate('RegisterVacance');
      ...
      this.props.navigation.setParams({
        num: (
          <View
            ...
            <Text style={{margin: 5, color: '#ccc'}}>
              {this.state.selected.length}
            </Text>
            <TouchableOpacity onPress={this.Save}>
              <Text style={{color: 'white', fontSize: 16, marginRight: 18}}>
                SALVAR
              </Text>
            </TouchableOpacity>
          </View>
...
const mapStateTopProps = state => ({
  images: state.Properties.images,
});
export default connect(
  mapStateTopProps,
  {SavePhotoCam, SavePhotos},
)(SelectPhotos);
